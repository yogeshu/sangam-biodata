    ...
    <CommonLayout showFooter={false}>
      <div className="min-h-screen bg-background-light dark:bg-background-dark text-text-main font-body">
        <div className="mx-auto flex max-w-5xl flex-col gap-8 px-4 pb-16 pt-8 md:px-8 lg:pt-12">
          ...
          <div className="rounded-2xl border border-border-soft dark:bg-midnight-accent shadow-sm">
          ...
          {data.partnerExpectationsMode === 'detailed' ? (
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                className={cn(
                  "w-full rounded-lg border px-3 py-2.5 text-sm dark:bg-midnight-accent text-text-main outline-none focus:border-primary focus:ring-1 focus:ring-primary transition resize-none",
                ...
                <div className="flex-1 space-y-1">
                  ...
                  <div className="flex gap-2">
                    <select
                      ...
                      className="rounded-lg border border-border-soft px-3 py-2.5 text-sm dark:bg-midnight-accent text-text-main outline-none focus:border-primary focus:ring-1 focus:ring-primary transition w-32"
                    >
                      <option>Self</option>
                    ...
                    <input
                      ...
                      className="flex-1 rounded-lg border border-border-soft px-3 py-2.5 text-sm dark:bg-midnight-accent text-text-main outline-none focus:border-primary focus:ring-1 focus:ring-primary transition"
                    />
...
function InputField({ label, name, value, onChange, placeholder, type = "text", icon, optional, error }: { label: string; name: string; value: string; onChange: (e: ChangeEvent<HTMLInputElement>) => void; placeholder?: string; type?: string; icon?: ReactNode; optional?: boolean; error?: string }) {
  return (
    <label className="flex flex-col gap-1.5 text-sm font-medium text-text-main">
      ...
      <div className={cn("flex items-center gap-2 rounded-lg border px-3 py-2.5 focus-within:border-primary focus-within:ring-1 focus-within:ring-primary transition dark:bg-midnight-accent", error ? "border-red-500 focus-within:ring-red-500 focus-within:border-red-500" : "border-border-soft")}>
...
function SelectField({ label, name, value, onChange, placeholder, options, icon, optional, error, touched: isTouched }: { label: string; name: string; value: string; onChange: (e: ChangeEvent<HTMLSelectElement>) => void; placeholder?: string; options: string[]; icon?: ReactNode; optional?: boolean; error?: string; touched?: boolean }) {
  return (
    <label className="flex flex-col gap-1.5 text-sm font-medium text-text-main">
      ...
      <div className={cn("flex items-center gap-2 rounded-lg border px-3 py-2.5 focus-within:border-primary focus-within:ring-1 focus-within:ring-primary transition dark:bg-midnight-accent", error && isTouched ? "border-red-500 focus-within:ring-red-500 focus-within:border-red-500" : "border-border-soft")}>
        {icon && <span className="text-text-muted">{icon}</span>}
        <select
          name={name}
          value={value}
...
function HelperHint({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2 rounded-lg border border-dashed border-border-soft dark:bg-midnight-accent px-4 py-3 text-sm text-text-muted">
      <CheckCircle2 size={14} className="text-primary" />
      <span>{children}</span>
    </div>
  );
        ...
        {fields.map((field, index) => (
          <div key={index} className="flex gap-2 items-start">
            <input
              ...
              className="flex-1 px-3 py-2 text-sm border border-border-soft rounded-lg dark:bg-midnight-accent text-text-main focus:border-primary focus:ring-1 focus:ring-primary outline-none"
            />
            <input
              type="text"
